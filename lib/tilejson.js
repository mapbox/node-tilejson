    ...
    return function(once) {
        if (!locks[id].io) {
            ...
            once(function(err, data) {
                console.log('PRE-EMIT listeners.length', locks[id].listeners('done').length);
                locks[id].emit('done', err, data);
                console.log('EMIT listeners.length', locks[id].listeners('done').length);
                delete locks[id];

...
var RetryAgent = undici.RetryAgent;
var fetch = undici.fetch;
...
var retryAgent = new RetryAgent(agent);
...
function TileJSON(uri, callback) {
    ...
    if (templateToTileJSON(formatted)) {
        ...
        return lock(function(callback) {
            ...
            tilejson.get(formatted, function(err, json) {
                if (err && (err.statusCode === 403 || err.statusCode === 404 || err.statusCode === 410))
...
TileJSON.prototype.getTile = function(z, x, y, callback) {
    ...
    this.get(url, function(err, data, headers) {
        if (err && (err.statusCode === 404 || err.statusCode === 403 || err.statusCode === 410))
...
TileJSON.prototype.get = function(url, callback) {
    fetch(url, {
        ...
        res.json().then(function(data) {
            callback(null, data, res && res.headers);
        })
        .catch(function(jsonErr) {
            console.log(jsonErr);
            console.log(res);
            console.log('asddsfdsafdsafdsfdasfsdafasdf');

            callback(jsonErr, '', res && res.headers);
        });
    }).catch(function(err) {
        console.log(err);

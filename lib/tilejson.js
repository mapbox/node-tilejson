...
TileJSON.prototype._open = function(uri) {
    ...
    Step(function() {
        ...
        if (!tilejson.data.id) {
            tilejson.data.id = path.basename(uri.pathname, '.tilejson');
        }

        // Create an MBTiles object as cache.
        var cacheFile = uri.pathname + '.cache';
        tilejson.cache = new sqlite3.cached.Database(cacheFile, this);
    }, function(err) {
        if (err) return error(err);
...
TileJSON.prototype._downloadGrid = function(z, x, y, callback) {
    ...
    new get(url).asString(function(err, grid, headers) {
        if (err) return callback(new Error('Grid unavailable'));

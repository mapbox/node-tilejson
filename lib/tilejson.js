...
function TileJSON(uri, callback) {
    ...
    function loaded(err, data) {
        ...
        // Create an MBTiles object as cache.
        var cacheFile = uri.pathname + '.cache';
        this.cache = new MBTiles(cacheFile, function(err) {
            if (err) callback(err);
...
TileJSON.prototype.getTile = function(z, x, y, callback) {
    ...
    var tilejson = this;
    this.cache.getTile(z, x, y, function(err, tile, headers) {
        if (!err) callback(err, tile, headers);
        else tilejson._cacheTile(z, x, y, callback);
    });
};

TileJSON.prototype._cacheTile = function(z, x, y, callback) {
    var cache = this.cache;

    var url = this.data.tiles[0]
        .replace(/\{z\}/g, z)
        .replace(/\{x\}/g, x)
        .replace(/\{y\}/g, (this.data.scheme === 'tms') ? Math.pow(2, z) - 1 - y : y);

    new get(url).asBuffer(function(err, data, headers) {
        if (err) return callback(new Error('Tile unavailable'));
